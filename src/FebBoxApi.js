...
class FebboxAPI {
    ...
    async getQuota(cookie = null) {
        ...
        if (resp.data && resp.data.flow) {
            const flow = resp.data.data ? resp.data.data.flow : resp.data.flow
            const remaining = (Number(flow.traffic_limit_mb) || 0) - (Number(flow.traffic_usage_mb) || 0)
            return { ok: true, remainingMB: remaining , totalMB: Number(flow.traffic_limit_mb)}
        }
        return { ok: false, remainingMB: -1 , totalMB: 0}
    }
